...
func (s *StateDB) Commit(block uint64, deleteEmptyObjects bool, opts ...Option) (common.Hash, error) {
	...
	if root != origin {
		...
		if err := s.db.TrieDB().Update(root, origin, block, nodes, set, FilterTrieDBOptions(opts)...); err != nil {
			return common.Hash{}, err
		}
		s.originalRoot = root
		if metrics.EnabledExpensive {
