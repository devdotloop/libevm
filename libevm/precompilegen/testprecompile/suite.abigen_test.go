...
func DeployTestSuite(auth *bind.TransactOpts, backend bind.ContractBackend, _precompile common.Address) (common.Address, *types.Transaction, *TestSuite, error) {
	...
	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(TestSuiteBin), backend, _precompile)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
